...
const StudentDashboardLayout = () => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex">
      ...
      {/* SIDEBAR */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#2D3162] text-white transition-transform duration-300 transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static lg:inset-0`}
      >
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <header className="bg-white/80 backdrop-blur-md sticky top-0 z-30 border-b border-gray-100 h-24 flex items-center justify-between px-8 lg:px-12">
          ...
          <div className="flex items-center gap-6 ml-auto">
            <div className="hidden md:flex flex-col items-end">
              <p className="text-[#2D3162] font-black text-sm">
                {profile?.first_name} {profile?.last_name}
              </p>
              <span className="bg-orange-50 text-[#F97316] text-[10px] font-bold uppercase px-2 py-0.5 rounded-md">
                {profile?.role}
              </span>
            </div>
            <Link
              to="/student/profile"
              className="w-12 h-12 rounded-full border-2 border-white ring-2 ring-gray-50 overflow-hidden"
            >
              <img
                src={
        ...
        <main className="flex-1 overflow-y-auto p-6 lg:p-12">
          <div className="max-w-6xl mx-auto">
            ...
            <Outlet
              context={{ profileData: profile, refreshProfile: fetchProfile }}
            />
          </div>
        </main>
      </div>
